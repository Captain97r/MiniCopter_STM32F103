...
void accelerometer_calibration()
{
	...
	float a[3][3];
		for (uint8_t i = 0; i < 3; i++)
			for (uint8_t j = 0; j < 3; j++)
				a[i][j] = accel_raw_ref[i * 2 + 1][j] - accel_offs[j];
	
	// TODO: implement an inverse matrix function and do a^-1 * b
	// Determinant
	double det_a = (a[0][0]*a[1][1]*a[2][2] + a[0][1]*a[1][2]*a[2][0] + a[1][0]*a[2][1]*a[0][2]) - (a[0][2]*a[1][1]*a[2][0] + a[0][1]*a[1][0]*a[2][2] + a[2][1]*a[1][2]*a[0][0]);
