...
nRF24_RXResult nRF24_ReadAckPayload(uint8_t *pBuf, uint8_t length) {
	uint8_t pipe;
...
void nRF24_EnableAutoAck()
{
	nRF24_WriteReg(nRF24_REG_FEATURE, 0b00000110);
	nRF24_WriteReg(nRF24_REG_DYNPD, 0b00000011);
}

void nRF24_WriteAckPayload(uint8_t pipe, uint8_t *pBuf, uint8_t length) {
	nRF24_WriteMBReg(nRF24_W_ACK_PAYLOAD | pipe, pBuf, length);
}

uint8_t nRF24_isAckAvailable() {
	return !(nRF24_ReadReg(nRF24_REG_FIFO_STATUS) & 0x01);
}
