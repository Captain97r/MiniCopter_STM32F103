...
static void Hessenberg_Elementary_Transform(double *H,
	double* S,
	int perm[],
	int n)
{
...
static void Product_and_Sum_of_Shifts(double *H,
	int n,
	int max_row,
	double* shift,
	double *trace,
...
static void Update_Transformation(double *S,
	double cos,
	double sin,
	int n,
	int k)
...
static void Double_QR_Step(double *H,
	int min_row,
	int max_row,
	int min_col,
	double trace,
...
static void Double_QR_Iteration(double *H,
	double *S,
	int min_row,
	int max_row,
	int n,
...
static void One_Real_Eigenvalue(double Hrow[],
	double eigen_real[],
	double eigen_imag[],
	int row,
	double shift)
...
static void Two_Eigenvalues(double *H,
	double* S,
	double eigen_real[],
	double eigen_imag[],
	int n,
...
static void BackSubstitute_Real_Vector(double *H,
	double eigen_real[],
	double eigen_imag[],
	int row,
...
static void Complex_Division(double x,
	double y,
	double u,
	double v,
	double* a,
...
static void BackSubstitute_Complex_Vector(double *H,
	double eigen_real[],
	double eigen_imag[],
	int row,
	double zero_tolerance,
...
static void BackSubstitution(double *H,
	double eigen_real[],
	double eigen_imag[],
	int n)
{
...
static void Calculate_Eigenvectors(double *H,
	double *S,
	double eigen_real[],
	double eigen_imag[],
	int n)
...
int mag_calibrate()
{
	...
	double maxval, norm, btqb, hm, norm1, norm2, norm3, J, hmb;
	double x, y, z;
	
	//int8_t nlines = 100;

	int16_t raw[3];
	double D[100 * 10 * sizeof(double)];
	
	for (uint16_t i = 0; i < 100; i++)
	{
		MPU9250_get_mag_raw(raw);
	...
	double S[10 * 10 * sizeof(double)];
	Matrix_x_Its_Transpose(S, D, 10, 100);
 
	// Create pre-inverted constraint matrix C
	double C[6 * 6 * sizeof(double)];
	...
	double S11[6 * 6 * sizeof(double)];
	Get_Submatrix(S11, 6, 6, S, 10, 0, 0);
	double S12[6 * 4 * sizeof(double)];
	Get_Submatrix(S12, 6, 4, S, 10, 0, 6);
	double S12t[4 * 6 * sizeof(double)];
	Get_Submatrix(S12t, 4, 6, S, 10, 6, 0);
	double S22[4 * 4 * sizeof(double)];
	Get_Submatrix(S22, 4, 4, S, 10, 6, 6);
 
	double S22_1[4 * 4 * sizeof(double)];
	for (i = 0; i < 16; i++)
		S22_1[i] = S22[i];
	...
	double S22a[4 * 6 * sizeof(double)];
	Multiply_Matrices(S22a, S22_1, 4, 4, S12t, 6);
 
	// Then calculate S22b = S12 * S22a      ( 6x6 = 6x4 * 4x6)
	double S22b[6 * 6 * sizeof(double)];
	Multiply_Matrices(S22b, S12, 6, 4, S22a, 6);
 
	// Calculate SS = S11 - S22b
	double SS[6 * 6 * sizeof(double)];
	for (i = 0; i < 36; i++)
		SS[i] = S11[i] - S22b[i];
	double E[6 * 6 * sizeof(double)];
	Multiply_Matrices(E, C, 6, 6, SS, 6);
	double SSS[6 * 6 * sizeof(double)];
	Hessenberg_Form_Elementary(E, SSS, 6);

	double eigen_real[6 * sizeof(double)];
	double eigen_imag[6 * sizeof(double)];
	...
	double v2[4 * sizeof(double)];
	Multiply_Matrices(v2, S22a, 4, 6, v1, 1);

	double v[10 * sizeof(double)];
	...
	double SSSS[3 * 3 * sizeof(double)];
	Hessenberg_Form_Elementary(Q, SSSS, 3);

	double eigen_real3[3 * sizeof(double)];
	double eigen_imag3[3 * sizeof(double)];
